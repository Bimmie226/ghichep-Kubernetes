...
Ta cần 1 sv LB nằm ngoài cụm để forward traffic từ client đến NodePort của k8s 
...
```bash
vim /etc/nginx/conf.d/luongvn.com.conf
```

```nginx
upstream my_servers {
  server 192.168.174.101:30080; 
  server 192.168.174.102:30080; 
  server 192.168.174.103:30080;
}
...
![alt text](../images/ingress_04.png)

Chọn Service Discovery -> Ingresses -> Sau đó chọn import file yaml mà ta đã lưu sau khi triển khai ingress nginx ở bước trên 
...
- `ingressClassName`: Cho biết ingress này được xử lý bởi Ingress Controller nào
- `rules`: danh sách các luật định tuyến 
- `host`: Tên miền mà Ingress sẽ xử lý.
- `http`: khai báo các luật http 
  
![alt text](../images/ingress_05.png)

![alt text](../images/ingress_06.png)

![alt text](../images/ingress_07.png) 

> Ingress đã forward qua Service và sau đó fw qua Pod 


### Kiểm tra

Add host: `nginx-onpre.luongvn.com` sau đó kiểm tra trên browser

![alt text](../images/ingress_08.png) 